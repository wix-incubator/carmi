...
const exprHash_ignoreLiterals = memoizeNonPrimitives(
  obj => {
    // console.log( Array.isArray(obj), _.isPlainObject(obj),JSON.stringify(obj, null,2))
    if (Array.isArray(obj)) {
      return objectHash(_.map(obj, val => exprHash_ignoreLiterals(val)).join(','));
    } else if (_.isPlainObject(obj)) {
      const keys = Object.keys(obj).sort();
      return objectHash(_.map(keys, key => `${key}:${exprHash_ignoreLiterals(obj[key])}`).join(','));
    } else if (_.isFunction(obj)) {
      throw new TypeError(`Trying to chain a function in carmi code: ${obj}`)
    } else {
      return hashString(JSON.stringify(obj));
...
module.exports = {exprHash, exprHash_ignoreLiterals, hashString, clearHashStrings};
