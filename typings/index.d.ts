...
> = unknown extends A
  ...
  : unknown extends B
  ? (F extends (a: A, ...args: infer Args) => infer R
      ? (...args: Args) => R
      : never)
  : unknown extends C
  ? (F extends (a: A, b: B, ...args: infer Args) => infer R
      ? (...args: Args) => R
      : never)
  : unknown extends D
  ? (F extends (a: A, b: B, c: C, ...args: infer Args) => infer R
      ? (...args: Args) => R
      : never)
  : unknown extends E
  ? (F extends (a: A, b: B, c: C, d: D, ...args: infer Args) => infer R
      ? (...args: Args) => R
      : never)
  : never;

export interface BoolGraph<F extends FunctionLibrary>
  extends GraphImpl<boolean, F> {}
export interface FunctionGraph<N, F extends FunctionLibrary>
  extends GraphImpl<N, F> {}

/**
 * Graph
 */
interface GraphImpl<NativeType, F extends FunctionLibrary>
  extends GraphBase<NativeType> {
  /**
  ...
  call<
    ...
    Arguments extends F[FunctionName] extends (
      firstArg: NativeType,
      ...args: infer Args
    ) => any
      ? Args
   ...
   */
  bind<FunctionName extends keyof F>(
    func: FunctionName
  ): FunctionGraph<BoundFunction<F[FunctionName], NativeType>, F>;
   ...
   * @param condition
   * @sugar */
  conditionalBreakpoint<FunctionName extends keyof F>(
  ...
  switch<DefaultCase, TupleType extends [Argument<NativeType>, any]>(
    ...
    | Graph<DefaultCase, F>
    | Graph<
        TupleType extends [Argument<NativeType>, infer Result] ? Result : never,
        F
      >;
 ...
 * Number
 */
export interface NumberGraph<
  NativeType extends number,
...
> extends GraphImpl<NativeType, F> {
   ...
   */
  range(
    start?: Argument<number>,
    skip?: Argument<number>
  ): NumberGraph<number, F>[];
 ...
 * Array or Object
 */
interface ArrayOrObjectGraphImpl<
  NativeType extends any[] | object,
  F extends FunctionLibrary,
...
> extends GraphImpl<NativeType, F> {
  ...
  get<K extends keyof NativeType>(
    key: K | AbstractGraph
  ): K extends AbstractGraph
    ? Graph<NativeType[keyof NativeType], F>
    : Graph<NativeType[K], F>;
   ...
   * @param path
   * @sugar */
  getIn<K extends keyof NativeType>(
    path: [Argument<K>]
  ): Graph<NativeType[K], F>;
...
> extends ArrayOrObjectGraphImpl<Value[], F> {
   ...
   */
  filter<Scope>(
    functor: (value: ValueGraph, key: KeyGraph, scope: Scope) => any,
    scope?: Scope
  ): ArrayGraph<Value[], F>;
  ...
  ): Value extends string ? StringGraph<string, F> : never;
...
>
   ...
   * @param value
   * @sugar */
  assignIn<V extends object>(
    value: Argument<V>[]
  ): ObjectGraph<NativeType & AsNative<V>, F>;
   ...
   * @param scope
   */
  recursiveMapValues<Scope, Functor extends (
    loop: Looper<Ret>,
    value: ValueGraph,
    key: KeyGraph,
    scope: Scope
  ) => Argument<Ret>, Ret = ReturnType<Functor>>(
    functor: Functor,
    scope?: Scope
  ): ObjectGraph<
    {
...
export interface ArrayGraph<T extends any[], F extends FunctionLibrary>
  extends ArrayGraphImpl<T, F> {}
export interface ObjectGraph<T extends object, F extends FunctionLibrary>
  extends ObjectGraphImpl<AsNative<T>, F> {}
 ...
 * External
 */
export interface CarmiAPI<
  Schema extends object = any,
  F extends FunctionLibrary = any
> {
  $schema: Schema;
  $functions: F;
  ...
  or<A, B, C, D>(
    ...
    c: C,
    d: D
  ): Graph<A, F> | Graph<B, F> | Graph<C, F> | Graph<D, F>;
  or<A, B, C, D, E>(
    a: A,
    b: B,
    c: C,
    d: D,
    e: E
  ): Graph<A, F> | Graph<B, F> | Graph<C, F> | Graph<D, F> | Graph<E, F>;
  or<A, B, C, D, E, FF>(
    a: A,
    b: B,
    c: C,
   ...
   */
  setter<Path extends PathSegment[]>(
    ...path: Path
  ): SetterExpression<Schema, Path, F>;

  /**
   * Declare actions which can be triggered on your state to change it (use arg0/arg1/arg2 - to define placeholders in the path).
   */
  splice<Path extends PathSegment[]>(
    ...path: Path
  ): SpliceExpression<Schema, Path, F>;

  /**
   * Declare a setter that adds an element to the end of an array. The setter will create the array if one doesn't exist.
   */
  push<Path extends PathSegment[]>(
    ...path: Path
  ): PushExpression<Schema, Path, F>;
  ...
  bind<FunctionName extends keyof F>(
    func: FunctionName
  ): FunctionGraph<BoundFunction<F[FunctionName]>, F>;
  bind<FunctionName extends keyof F, A>(
    func: FunctionName,
    a: A
   ...
   */
  template<F extends FunctionLibrary>(
...
export function withSchema<
  ...
  F extends FunctionLibrary
>(model?: Schema, functions?: F): CarmiAPI<Schema, F>;
